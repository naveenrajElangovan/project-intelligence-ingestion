...
def test_lab_1_uses_production_chunker() -> None:
    module = runpy.run_path(str(Path(__file__).parents[2] / "labs/lab01_langchain_chunking.py"))
    result = module["run"]()
    assert result
...
def test_lab_2_builds_transient_binary_document(tmp_path: Path) -> None:
    module = runpy.run_path(str(Path(__file__).parents[2] / "labs/lab02_docling.py"))
